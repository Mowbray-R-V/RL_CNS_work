...
class Buffer:
    ...
    def learn(self):
        ...
        phase_state_mod, control_state_mod, nxt_phase_state_mod, nxt_control_state_mod = parallel_process(
            self.state_buffer[batch_indices], 
            self.next_state_buffer[batch_indices], 
            batch_indices
        )

        print(f"[DDPG.py]: state_time: {round(time.time() - state_time, 3)}")
        ...
        # Check for NaNs
        assert not tf.reduce_any(tf.math.is_nan(control_state)), "Control state has NaNs."
        assert not tf.reduce_any(tf.math.is_nan(phase_state)), "Phase state has NaNs."
        # Further checks...

        learn_init_time = time.time()
        self.update(control_state, phase_state, action_batch, reward_batch, next_control_state_batch, next_phase_state_batch)
        print(f"[DDPG.py]: learning time: {round(time.time() - learn_init_time, 3)}")
