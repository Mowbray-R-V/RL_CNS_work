...
# sys.path.append(os.path.abspath('../safe_rl_code'))
from rl_modules.parallel_processing import parallel_process
# from tensorflow.keras.utils import plot_model




# graph_flag = True # False #
...
class NNModel(tf.keras.Model): 
    ...
    def call(self, control_state = None, phase_state = None, action = None, act_or_cri = 'act'):
        # assert features.shape[0] == config.rl_ddpg_samp_size or features.shape[0] == 1, f'wrong dimensions, input_batch: {features.shape[0],features.shape}'
...
class ActorModel(tf.keras.Model):
    def __init__(self):
        ...
        # self.reshape_con = layers.Reshape(target_shape=(config.num_veh, config.state_size))
        # self.repeat = layers.RepeatVector(config.num_veh)
        self.conact  = layers.Concatenate(axis = -1)

        self.flat = layers.Flatten()
        # self.reshape_obs = layers.Reshape(target_shape=(config.num_veh, config.state_size))

        ###########original model##################
        # self.layer_conv2004 = CNN(2004, config.num_veh*config.state_size, config.num_veh*config.state_size, activation="relu")
        ...
        self.layer_conv2004p = CNN(2004, config.num_veh*config.state_size, config.num_veh*config.state_size, activation="relu")
    ...
    def call(self, control_state, phase_state, mask=None):
        ...
        # print(f'phase_scores:{phase_scores.shape}, va;l:"{phase_scores}')
        phase_scores_norm = self.unit(phase_scores)
        # print(f'\n phase_scores_norm:{phase_scores_norm.shape}, val:{phase_scores_norm}')
        # exit()
        phase_scores_norm_dim = tf.expand_dims(phase_scores_norm, axis=-2)#### check compute graph
        phase_scores_num = tf.repeat(phase_scores_norm_dim,repeats=config.num_veh, axis=1)   #### check compute graph
        ######phase_module#########
       ...
       ######control_module#########
        ...
        control_state_num = tf.repeat(control_state_1,repeats=config.num_veh, axis=1)   #### check compute graph
        ...
        obs_state  =  tf.reshape(control_state, [control_state.shape[0],config.num_veh, config.state_size])
...
class DDPG:
    ...
    def __init__(self, sim=0, noise_mean=0, noise_std_dev=0.2, cri_lr=0.001, act_lr=0.0001, disc_factor=0, polyak_factor=0, buff_size=1000, samp_size=64):
        ...
        self.num_states =  config.state_size*config.num_veh   ##### need to edit
        self.num_obs = config.num_features
        ...
        sample_batch = 1
        build_ouput_act = self.model(control_state = tf.random.uniform((sample_batch,config.num_veh*config.state_size)), phase_state=tf.random.uniform((sample_batch, config.num_phases,(2*config.max_robot_lane),config.state_size)), act_or_cri='act')
        # exit()
        build_ouput_crtic = self.model(control_state=tf.random.uniform((sample_batch,config.num_veh*config.state_size)), phase_state=None, action = tf.random.uniform((sample_batch,self.num_actions)),act_or_cri='critic')
        # exit()
        ...
        # build_ouput = self.model(control_state = None, phase_state=tf.random.uniform((1,config.num_phases,(2*config.max_robot_lane),config.state_size)), act_or_cri='act')
        # build_ouput_1 = self.model(control_state=tf.random.uniform((1,config.num_veh ,config.state_size)), phase_state=None, action = tf.random.uniform((1,self.num_actions)),act_or_cri='critic')
    ...
    def policy(self, state_control, state_phase, time, noise_object, num_veh=None):
        ...
        legal_action = self.sigmoid_func(sampled_actions[config.num_phases:(self.num_actions)])
        legal_action_ph = sampled_actions[:config.num_phases] 
        ...
        assert all([ _>=0 and _<=1  for _ in legal_action_set[config.num_phases:(self.num_actions)]]),f'alpha value not in range :{legal_action_set[config.num_phases:(self.num_actions)]}, \
            signal:{legal_action_set[:config.num_phases]} \n\n, state_contr:{state_control}'
...
class Buffer:
    ...
    def learn(self):
        ...
        phase_state_mod = np.zeros((np.shape(self.state_buffer[batch_indices])[0], config.num_phases, (config.max_robot_lane*2), config.state_size))
        control_state_mod = np.zeros((np.shape(self.state_buffer[batch_indices])[0], config.num_veh*config.state_size))

        nxt_phase_state_mod = np.zeros((np.shape(self.next_state_buffer[batch_indices])[0], config.num_phases, (config.max_robot_lane*2), config.state_size))
        nxt_control_state_mod = np.zeros((np.shape(self.next_state_buffer[batch_indices])[0], config.num_veh*config.state_size))
