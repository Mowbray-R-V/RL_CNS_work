...
import numpy as np ;  from numpy.random import seed
import matplotlib; matplotlib.use('Agg')
import matplotlib.pyplot as plt
from collections import deque; import pickle ; import cloudpickle

from utility import config, vehicle, functions, set_class, file_init
from rl_modules import contr_sig_rl
...
def func(_args):
    ...
    version = _args[4]
    train_sim = _args[5]    
    algo_option = config.algo_option 
    ...
    signal_ts = {key: None for key in range(1, config.max_sim_time+1)}
    override_ts = {key: None for key in range(1, config.max_sim_time+1)}  
    # problem == 'eight lane'	

    if config.problem == 'single lane':
        selected_phase = {1: 'R', 2: 'R', 4: 'G', 5: 'G', 7: 'R', 8: 'R', 10: 'R', 11: 'R' } 
    ...
    if config.rl_flag:
        ...
        if config.rl_algo_opt == "DDPG":
            from rl_modules.DDPG import DDPG as Agent    #DDPG_model_seq_v_one import DDPG as Agent
        elif config.rl_algo_opt == "MADDPG":
            from rl_modules.maddpg import DDPG as Agent
        ...
        ss = [config.buff_size, 64] # buffer size and sample size
        actor_lr = 0.0001
        critic_lr = 0.001
        p_factor = 0.0001
        d_factor = 0
        ...
        if config.rl_algo_opt == "DDPG":
            if algo_option == "rl_modified_ddswa":
                ...
                agent = Agent(algo_opt=algo_option, state_size=config.num_features*config.lane_max, action_size=2*config.lane_max)
        elif config.rl_algo_opt == "MADDPG": pass
		 

    ## load trained model
    if (not learning_flag) and (not config.env_test): 
    ...
    if config.file_analysis: file_init.file_list()
    ...
    for rep_sim in range(0, max_rep_sim): 
        ...
        throughput_id_in_lane = [0 for _ in config.lanes] # variable to help track last vehicle in each lane, a list of zeros, change to 1 if particular one crosses the intersection
        sim_obj = set_class.sets() 


        if config.real_time_spawning_flag:

            ############# lane independent ID ###############
            veh_id_var = 0

            ############# lane dependent ID ###############
            dep_veh_id  = [(100*lane) for lane in config.lanes] 

            next_spawn_time = [100 + config.max_sim_time for _ in config.lanes]  
        ...
        ### start of simulation ###
        while (time_track < (config.max_sim_time)+1):  

            curr_time = time.time()

            if config.rl_algo_opt == "DDPG" and learning_flag and (agent.buffer.buffer_counter > 0) and ((time_track % 1) == 0):
            ...
            for lane in config.lanes: 
                if (config.real_time_spawning_flag) and (round(time_track, 1) >= round(next_spawn_time[lane], 1)) and (len(sim_obj.unspawned_veh[lane]) == 0) and (not (arr_rate_array[lane] == 0)):
                    ...
                    next_spawn_time[lane] = round(time_track + float(np.random.exponential(1/arr_rate_array[lane],1)), 1)
                    new_veh = vehicle.Vehicle(lane, config.int_start[lane], 0, config.vm[lane], config.u_min[lane], config.u_max[lane], config.L, arr_rate_array)
                    # next_spawn_time[lane] = round(time_track + float(np.random.exponential(1/arr_rate_array[lane],1)), 1)
                    # new_veh = vehicle.Vehicle(lane, config.int_start[lane], 0, config.vm[lane], config.u_min[lane], config.u_max[lane], config.L, arr_rate_array)
                    ...
                    new_veh.goal_dist =  copy.deepcopy(config.int_start[lane])   ###to update
                ...
                while v_itr<n:
                    ...
                    if (round(v.sp_t,1) < round(time_track,1)) and (functions.check_init_config(v, pre_v, time_track)): 
                        ...
                        # encode_flag = 0
                        for phase_iter in range(len(config.phase_ref)):
                            # for _ in config.lanes:
                            if config.phase_ref[phase_iter] == dict_sig:
            ...
            functions.seq_checker(sim_obj.unspawned_veh)
            functions.seq_checker(sim_obj.spawned_veh)
            for _ in config.lanes: assert(sorted(list(set([v.id for v in (sim_obj.spawned_veh[_])]))) ==  [v.id for  v in (sim_obj.spawned_veh[_])]), \
            ...
            if functions.get_num_of_objects(sim_obj.spawned_veh) > 0 :
                ...
                if  not config.env_test: phase_explore.append(phase_id) #DTCS
                if learning_flag:
                ...
                ### END - RL decision ###
                    ...
                    if functions.get_num_of_objects(sim_obj.spawned_veh)>0:
                        ...
                        curr_rew -= sum([v.priority*(-v.p_traj[-1] + v.intsize) for _ in config.lanes for v in sim_obj.spawned_veh[_]])
                    else: curr_rew = 0	
                    ########### Penalty ###########
                ...
                # 	dict_sig_copy = copy.deepcopy(dict(config.phase_ref[np.random.randint(0,12)]))
                # 	# print(f'\n config_signal value:{dict_sig}')
                ...
                if config.output =='Signal': 
                    assert len(dict_sig) == config.num_lanes*4 
                elif config.output =='Phase': #len(dict_sig) == config.num_phases
                    assert len(dict_sig) == config.num_lanes*4, f'dict:{len(dict_sig)},phase:{config.num_lanes*4}'


                ########### CHECKING MODULE ###########
                assert all ([v.id == spawned_veh_copy[_][iter].id for _ in config.lanes for iter, v in enumerate(sim_obj.spawned_veh[_])]),f'spawned set passed from RL WITH ERROR'
                ...
                for _ in override_lane: 
                    ...
                    for lane in config.incompdict[_]:
                        dict_sig[lane]='R'
                ######### phase override for delta time ##############

                for _ in override_lane:								
                    if dict_sig[_] == 'G' and config.file_analysis:  
                        f = open("over_green.txt", "a")
                        f.write(f'\n time:{time_track}, over ID in that lane:{override_lane[_]}, ID in lane:{[ veh.id for lane in config.lanes for veh in sim_obj.spawned_veh[lane]]}')
                        f.close()
                ...
                bisection_time = time.time()
                for lane in config.lanes:
                    # green_zone = -1*config.vm[lane]*config.dt + (config.vm[lane]**2)/(2*(max(config.u_min[lane],-(config.vm[lane]/config.dt))))  
                        ...
                        if dict_sig[lane]=='G':
                            ...
                            for iter in range(n):
                                if sim_obj.spawned_veh[lane][iter].pos_status == 'inter':  #urr_lane_veh, iter, num, time_track, signal
                                    ...
                                    if lane_sig_stat[lane][0] == 'G' and functions.buffer_inside_check(sim_obj.spawned_veh[lane][iter]):
                                    ...
                                    elif (lane_sig_stat[lane][0] == 'G' and not functions.buffer_inside_check(sim_obj.spawned_veh[lane][iter])) or lane_sig_stat[lane][0] == 'R':
                ...
                # 	for lane in config.lanes for veh in sim_obj.spawned_veh[lane]]),f'robot skippped---position exceeded,'
                ...
                for lane in config.lanes for count, veh in enumerate(sim_obj.spawned_veh[lane]) for index in range(count) ]),f'robot skippped---position exceeded,'

                print(f"[bisection_time]: {round(time.time() - bisection_time, 3)}")

                for _ in config.lanes:
                    for veh in sim_obj.spawned_veh[_]:
                        if  (config.L + veh.intsize) >= veh.p_traj[-1] > config.int_start[_]:
                        ...
                        elif (config.L + veh.intsize) >= veh.p_traj[-2] > config.int_start[_]: pass # assert False,f'pos:{veh.p_traj}, pos_status:{veh.pos_status}'
                        else: assert False
                ...
                for lane in config.lanes:
                    if len(config.incompdict[lane])>0:
                        ...
                        for iter in range(n):
                            ...
                            for phase_iter in config.phase_ref.keys(): #range(len([config.phase_ref.keys()][0])):
                                if config.phase_ref[phase_iter] == dict_sig:
                        ...
                        for veh_ in sim_obj.done_veh[lane]:
                            if functions.done_pos_check(veh_):    
                                ...
                                # print(f'\n ERRROR CHECK************** keys:{config.phase_ref.keys()}, len:{len([config.phase_ref.keys()][0])} ')
                                # exit()
                                for phase_iter in config.phase_ref.keys():  #range(len([config.phase_ref.keys()][0])):
                                    if config.phase_ref[phase_iter] == dict_sig:
            ...
            for lane in config.lanes:
                    ...
                    for iter in range(len(sim_obj.done_veh[lane])):
                        if functions.done_pos_check(sim_obj.done_veh[lane][iter]) and sim_obj.done_veh[lane][iter].pos_status == 'done':
                            ...
                            if functions.done_pos_check(sim_obj.done_veh[lane][iter]): 
                                sim_obj.done_veh[lane][iter].t_ser.append(round((time_track + (config.dt)), 1))
                ...
                if len(sim_obj.spawned_veh[lane])>0:		
                    ...
                    while ind < veh_num:
                        ...
                        if veh_iter.p_traj[-1] > (veh_iter.intsize + config.L) and veh_iter.pos_status == 'inter':         
                        # if veh_iter.p_traj[temp_ind] > (veh_iter.intsize + config.L) and veh_iter.pos_status == 'inter':  
                ...
                assert all([ veh.id not in override_lane_copy.values() and (veh.pos_status=='done' or veh.pos_status=='done_end')  for lane in config.lanes for veh in sim_obj.done_veh[lane] ])
            ...
            # 	for lane in config.lanes  for veh_ in sim_obj.spawned_veh[lane]]),f'error in spawn in done set,pos:' 
            ...
            ### intersection ###
            ### update current time###
            time_track = round((time_track + config.dt), 1)
            if learning_flag: #pass
                ...
                print("arr_rate:", arr_rate_,"current time:", time_track, "sim:", sim, "train_sim: ", train_sim, "train_iter:", train_iter, "arr_rate: ", arr_rate_,"********", end="\r")# "heuristic:", config.used_heuristic, "................", end="\r")
            ### update current time###
            ### throuput calculation ###
            for l in config.lanes:
                for v in sim_obj.spawned_veh[l]:
                    t_ind = functions.find_index(v, time_track)
                    if  ((t_ind == None) or (v.p_traj[t_ind] > (v.intsize + config.L))) and (v.id >= throughput_id_in_lane[l]):
            ...
            if (algo_option == "rl_modified_ddswa") and (learning_flag):
                if (not (prev_rew == None)) and (not (len(prev_state) == 0)):
                    ...
                    if rep_sim%10==0 and time_track == config.max_sim_time:
...
if __name__ == '__main__':
	...
	arr_rates_to_sim = config.arr_rates_to_simulate  #The 10 diff values from 0.01 to 0.1
	args = []
	...
	if config.used_heuristic == None:
		if config.rl_flag:
			...
			if train_or_test == "--train":
				...
				if not config.run_coord_on_captured_snap:
		...
		elif not config.run_coord_on_captured_snap:
			for _arr_rate_ in arr_rates_to_sim:
				arr_rate_array_ = {0:0, 1:_arr_rate_, 2:_arr_rate_, 3:0, 4:_arr_rate_, 5:_arr_rate_, 6:0, 7:_arr_rate_, 8:_arr_rate_, 9:0, 10:_arr_rate_, 11:_arr_rate_} #arr_rate*np.ones(len(lanes)) #
			...
			for _sim_num in range(1, 101): # 100 diff simulations
				for _train_sim in list(range(1)):
					for _arr_rate_ in arr_rates_to_sim:
						...
						heuristics_pickobj_save_path = f"../data/{config.used_heuristic}/arr_{_arr_rate_}/pickobj_sim_{_sim_num}"
