...
def get_state_medium(all_veh_set):   
	...
	for lane in data_file.lanes:
		...
		if len(data_file.incompdict[lane])>0:
			for veh in all_veh_set[lane]:
				...
				assert len(lane_state)  == data_file.state_size*(len(id_lane[lane]))
	...
	for _ in  data_file.lanes:
		if len(data_file.incompdict[_])>0:
			assert len(state[_]) == data_file.state_size*(len(id_lane[_])),f'LHS:{len(state[_])}, RHS:{data_file.state_size*(len(id_lane[_]))}, lane:{_}, id_lane:{id_lane}'
...
def rl_outputs(t_inst, _veh_set, rl_agent, alg_option, l_flag):
	...
	V_states = []
	Vp_set = _veh_set
	action_set =  np.zeros(data_file.num_actions)
	...
	if alg_option == "rl_modified_ddswa" and data_file.rl_algo_opt == "DDPG" :
		...
		####action extract#####
		assert np.size(rl_action) == data_file.num_actions,'action size wrong len'
		for i in range(data_file.num_phases, data_file.num_phases + data_file.num_veh): #for i in range(data_file.num_phases, functions.get_num_of_objects(Vp_set)+data_file.num_phases):
			alpha.append(rl_action[i])
		for i in range(data_file.num_phases +  data_file.num_veh, data_file.num_actions): #for i in range((data_file.num_phases + data_file.num_veh ), (functions.get_num_of_objects(Vp_set) + 2*data_file.num_veh )):
		...
		####action extract#####
		assert len(alpha) == data_file.num_veh,f'alpha size wrong len size:{len(alpha)}'
		assert len(beta) == data_file.num_veh,f'beta size wrong len size:{len(beta)}'
		assert len(phase) == data_file.num_phases,f'phase size wrong len size:{len(phase)}'
		...
		for act_elem in range(data_file.num_actions):
			# print(f'\nInside the controlsig assign: {act_elem},alpha:{len(alpha)}, beta:{len(beta)}')
			if act_elem in range(data_file.num_phases ):
				action_set[act_elem] = phase[act_elem] 
			elif act_elem  in range(data_file.num_phases,  data_file.num_veh + data_file.num_phases):
				action_set[act_elem] = alpha[act_elem - data_file.num_phases] 
			elif act_elem in range(data_file.num_phases + data_file.num_veh, data_file.num_actions):   #> data_file.num_phases + functions.get_num_of_objects(Vp_set) and alpha_pad_counter ==0:
				action_set[act_elem] = beta[act_elem- (data_file.num_phases + data_file.num_veh)] 
			else: assert False, {act_elem}


			# elif act_elem in range(data_file.num_phases + functions.get_num_of_objects(Vp_set),  data_file.num_phases + data_file.num_veh):   #> data_file.num_phases + functions.get_num_of_objects(Vp_set) and alpha_pad_counter ==0:
			# 	action_set[act_elem] =  alpha[act_elem - data_file.num_phases] #-190.392
			# elif act_elem in range(data_file.num_phases + data_file.num_veh + functions.get_num_of_objects(Vp_set),   data_file.num_actions):   #> data_file.num_phases + functions.get_num_of_objects(Vp_set) and alpha_pad_counter ==0:
			# 	action_set[act_elem] = beta[act_elem- (data_file.num_phases + data_file.num_veh)]   # -190.392

		# assert action_set.tolist().count(-190.392) == 2*(data_file.num_veh - functions.get_num_of_objects(Vp_set)),'action overwrite done wrong,'

		assert len(action_set) == (data_file.num_actions),f'{len(rl_action)},{(data_file.num_actions)}'
	...
	elif data_file.output =='Phase': return _veh_set, phase, dict_phase_, rl_state, np.zeros([1,data_file.num_features]) ,rl_action, exlporation_flag, phase_id
	# elif data_file.output =='Phase': return _veh_set, alpha, dict_alpha, beta, phase, dict_phase_, rl_state, np.zeros([1,data_file.num_features]) ,rl_action, exlporation_flag, phase_id