...
class ActorModel(tf.keras.Model):
    ...
    def call(self, obs, latent, mask,time_):
        ...
        with open('hidden_values_record.csv', 'a', newline='') as file:
            ...
            writer.writerow(obs[0][2].numpy().tolist())
        ...
        for iter1 in range(len(obs[0][2].numpy().tolist())):
            diff_obs.append(obs[0][2].numpy().tolist()[iter1] - prev_obs[iter1])
        ...
        f = open("debug.txt", "a")
        f.write(f'action_before_sigmoid:{control[0][2]}\n')
        f.close()
...
class DDPG:
    ...
    def policy(self, state, time, noise_object, num_veh=None):
        ...
        f = open("debug.txt", "a")
        f.write(f'action_after_sigmoid:{legal_action[2]}')
        f.close()
