...
def phase_encoding(veh, lane_info):
	...
	phase_encode = []
	for _ in range(len(data_file.lanes)):

		if data_file.phase_ref[_][lane_info] == 'G':
			phase_encode.append(1)
		elif data_file.phase_ref[_][lane_info] == 'R':
...
def phase_obs_transform(state, id_lane = None):
	...
	obs = {i:[] for i in data_file.lanes if len(data_file.incompdict[i])>0}  # Initialize a dictionary for lanes
	for lane_info in data_file.lanes:
		pad = 0
		if len(data_file.incompdict[lane_info])>0:

			obs[lane_info].extend(state[lane_info])
			if len(obs[lane_info])< data_file.max_robot_lane*data_file.state_size:
				assert isinstance(len(obs[lane_info]), int) and not(len(obs[lane_info]) % data_file.state_size)
				num_robot = int(len(obs[lane_info])/data_file.state_size)
				assert num_robot < data_file.max_robot_lane
				if id_lane != None: 
			...
			elif len(obs[lane_info])> data_file.max_robot_lane*data_file.state_size: assert False   #redundant check
			elif len(obs[lane_info]) == data_file.max_robot_lane*data_file.state_size: pass   #redundant check
			else : assert False
			...
			obs[lane_info] =  np.reshape(obs[lane_info],(data_file.max_robot_lane*data_file.state_size)) #obs[lane_info] =  np.reshape(obs[lane_info],(data_file.max_robot_lane, data_file.state_size))
	...
	assert np.shape(obs_req)[0] == data_file.num_veh*data_file.state_size    #assert np.shape(obs_req)[0] == data_file.num_veh and np.shape(obs_req)[1] == data_file.state_size
	return obs_req
...
def phase_state_transform(state, id_lane=None):
	...
	phase_state_dict = {i:[] for i in range(data_file.num_phases)}  
	phase_state_dict_req = {i:[] for i in range(data_file.num_phases)}  
	# obs_dict_req =  {i:[] for i in range(data_file.num_phases)}  

	for phase_iter in range(data_file.num_phases):
		counter = 0
		pad = 0
			...
			if len(data_file.incompdict[lane_info])>0:
				if data_file.phase_ref[phase_iter][lane_info] == 'G':
					...
					if len(state[lane_info])< data_file.max_robot_lane*data_file.state_size:
						assert isinstance(len(state[lane_info]), int) and not(len(state[lane_info]) % data_file.state_size),f'lane_state:{state[lane_info]}, bool_val{isinstance(len(state[lane_info]), int)}, nr:{len(state[lane_info])}, lane:{lane_info}   '
						num_robot = int(len(state[lane_info])/data_file.state_size)
						assert num_robot < data_file.max_robot_lane    ### redundant check
						if id_lane != None: 
							assert len(id_lane[lane_info]) == num_robot,f'LHS:{ len(id_lane[lane_info])}, RHS:{num_robot}, lane:{lane_info}'    ### redundant check
						pad +=  (data_file.max_robot_lane - num_robot)*data_file.state_size
		...
		phase_state_dict_req[phase_iter] = np.reshape(phase_state_dict[phase_iter],((2*data_file.max_robot_lane), data_file.state_size ))
		...
		# obs_dict_req[phase_iter] = np.reshape(obs_dict_req[phase_iter],((2*data_file.max_robot_lane), data_file.state_size ))
...
def intersection_control(curr_lane_veh, iter, num, time_track, learning_flag, signal):
	...
	if len(curr_lane_veh[iter].t_ser) >0: 
		...
		if signal == 'G':
			if (data_file.L + curr_lane_veh[iter].intsize) >= curr_lane_veh[iter].p_traj[temp_ind] > 0: pass    
			else: 
				assert False, "ERROR - agnet in next ROI requests for future position"
		elif signal == 'R':
			if (data_file.L + curr_lane_veh[iter].intsize) >= curr_lane_veh[iter].p_traj[temp_ind] > 0: pass    #### EXTRA checks the position 
			else: 
	...
	####alpha update avoided    #v.alpha = 1
...
def lane_control(curr_lane_veh, iter, num, time_track, signal, override_lane, lane, learning_flag):
	...
	if len(curr_lane_veh[iter].t_ser)>0: 
		...
		temp_ind = find_index(curr_lane_veh[iter], time_track)
		assert temp_ind!=None, f' time_track:{curr_lane_veh[iter].t_ser}'
		if 0 > curr_lane_veh[iter].p_traj[temp_ind] >= data_file.int_start[lane]: pass    
		else: 
...
def  done_pos_check(veh_):
	...
	return veh_.p_traj[-1] > veh_.intsize + data_file.L and veh_.p_traj[-1] < (veh_.intsize +  data_file.L - veh_.int_start/2) 
...
def max_physical_acc(veh_iter, time_int):
	...
	x_next = copy.deepcopy(compute_state(X, veh_iter.u_traj[temp_ind],round( data_file.dt,1),veh_iter.v_max))
