		...
		if data_file.rl_algo_opt == "DDPG":
			if algo_option == "rl_modified_ddswa":
				...
				if learning_flag:         
					...
					curr_obs = None
					prev_obs = None
					max_rep_sim =  500 # for CML 61 episodes of data, each of 500 sec [data collection phase]
		...
		while (time_track < (data_file.max_sim_time)+1):  
			...
			for lane in data_file.lanes: 
				if (data_file.real_time_spawning_flag) and (round(time_track, 1) >= round(next_spawn_time[lane], 1)) and (len(sim_obj.unspawned_veh[lane]) == 0) and (not (arr_rate_array[lane] == 0)):
					...
					new_veh.sig_stat_sym = copy.deepcopy(lane_sig_stat[lane][1]) 
					new_veh.sig_stat = 0
					new_veh.pos_status = 'lane'
					# new_veh.sig[time_track] = 'green'
			...
			if functions.get_num_of_objects(sim_obj.spawned_veh) > 0 :
				...
				sim_obj.spawned_veh, signal, dict_sig_copy, state_t, obs_t, action_t, explore_flag, phase_id = contr_sig_rl.get_phases(time_track, sim_obj.spawned_veh, agent, algo_option, learning_flag)
				...
				for lane in data_file.lanes:
					if len(spawned_veh[lane])>0:
						for v in spawned_veh[lane]:
							v.alpha = 1  #random.random() #0.25     
							# print(f'alpha:{v.alpha}')
				
				#phase_explore.append(phase_id) #DTCS
				if learning_flag:
					...
					if len(data_file.incompdict[lane])>0:
						...
						if dict_sig[lane]=='G':
							...
							for iter in range(n):
								...
								# veh_iter = sim_obj.spawned_veh[lane][iter]
								# sim_obj.spawned_veh[lane][iter] = functions.vehicle_initialise(sim_obj.spawned_veh[lane][iter])


								if sim_obj.spawned_veh[lane][iter].pos_status=='inter':
									
									temp_ind = functions.find_index(sim_obj.spawned_veh[lane][iter], time_track)
									assert temp_ind!=None,f'time_track:  {sim_obj.spawned_veh[lane][iter].t_ser}'
									if 0< sim_obj.spawned_veh[lane][iter].p_traj[temp_ind] <= (sim_obj.spawned_veh[lane][iter].intsize + data_file.L): 

										sim_obj.spawned_veh[lane][iter] = copy.deepcopy(functions.max_physical_acc(sim_obj.spawned_veh[lane][iter], time_track))
										sim_obj.spawned_veh[lane][iter].t_ser.append(round((time_track + (data_file.dt)), 1))
										if lane in override_lane:
											if sim_obj.spawned_veh[lane][iter].id  in override_lane[lane]: sim_obj.spawned_veh[lane][iter].sig[time_track] = 'orange_inter'
											elif sim_obj.spawned_veh[lane][iter].id  not in override_lane[lane]: sim_obj.spawned_veh[lane][iter].sig[time_track] = 'green_inter'
											else: assert False
										else: 
											sim_obj.spawned_veh[lane][iter].sig[time_track] = 'green_inter'

										if not learning_flag:functions.storedata(sim_obj.spawned_veh[lane][iter], train_sim, sim, train_iter, version) 
										
									else : 
										print('agnet in next ROI still in spawned_set')
										assert False

								elif sim_obj.spawned_veh[lane][iter].pos_status=='lane':

									if len(sim_obj.spawned_veh[lane][iter].t_ser)>0: 

										temp_ind = functions.find_index(sim_obj.spawned_veh[lane][iter], time_track)
										assert temp_ind!=None,f'time_track:  {sim_obj.spawned_veh[lane][iter].t_ser}'
										if 0>sim_obj.spawned_veh[lane][iter].p_traj[temp_ind]>=data_file.int_start[lane]:pass    #### EXTRA checks the position 
										else: 
									...
									if pre_v != None: assert pre_v.id < v.id,'robot ordre error in spawned set'
									sim_obj.spawned_veh[lane][iter], success = safe_set_map_bisect.acc_map(v, pre_v, time_track, flag = 'green')
									if not learning_flag:functions.storedata(sim_obj.spawned_veh[lane][iter], train_sim, sim, train_iter, version) 	
								else: 
									print('error')
									assert False,f'pos_status:{sim_obj.spawned_veh[lane][iter].pos_status}'

								# if time_track == sim_obj.spawned_veh[lane][iter].sp_t + 1: 
								# 	print(f'ID:{sim_obj.spawned_veh[lane][iter].id},time:{time_track}, spawn:{sim_obj.spawned_veh[lane][iter].sp_t} \n pos:{sim_obj.spawned_veh[lane][iter].p_traj}, u:{sim_obj.spawned_veh[lane][iter].u_traj},\n\nsig:{sim_obj.spawned_veh[lane][iter].sig} ')
								# 	exit()
								# assert sim_obj.spawned_veh[lane][iter].sig[time_track]!=None,f'RED \n ID:{sim_obj.spawned_veh[lane][iter].id}, time:{time_track}, \n signal_dict:{sim_obj.spawned_veh[lane][iter].sig},pos:{sim_obj.spawned_veh[lane][iter].pos_status}\n pos:{sim_obj.spawned_veh[lane][iter].p_traj}'

	
						elif dict_sig[lane]=='R':
							n = len(sim_obj.spawned_veh[lane])
							for iter in range(n):
								# assert sim_obj.spawned_veh[lane][iter].sig[time_track]!=None,f'RED \n ID:{sim_obj.spawned_veh[lane][iter].id}, time:{time_track}, \n signal_dict:{sim_obj.spawned_veh[lane][iter].sig},pos:{sim_obj.spawned_veh[lane][iter].pos_status}\n pos:{sim_obj.spawned_veh[lane][iter].p_traj},\n ovr:{override_lane[lane]}'
								...
								if sim_obj.spawned_veh[lane][iter].pos_status=='inter':
									...
									if 0< sim_obj.spawned_veh[lane][iter].p_traj[temp_ind]<= (sim_obj.spawned_veh[lane][iter].intsize + data_file.L): 
										...
										if sim_obj.spawned_veh[lane][iter].id in override_veh:		  
										...
										if lane in override_lane:
											...
											print('agnet in next ROI still in spawned_set')
											assert False,f'lane:{lane},id:{veh_iter.id},pos:{veh_iter.p_traj}, over:{override_lane},signal:{dict_sig},{sim_obj.spawned_veh[lane][iter].pos_status}'
											#veh_iter.sig[time_track] = 'green'
										#######################################################
										if not learning_flag:functions.storedata(sim_obj.spawned_veh[lane][iter], train_sim, sim, train_iter, version) 
										
									else: 
										print('agnet in next ROI still in spawned_set')
										assert False,f'lane:{lane},id:{sim_obj.spawned_veh[lane][iter].id},pos:{sim_obj.spawned_veh[lane][iter].p_traj},over: {override_lane}'
									...
									if len(sim_obj.spawned_veh[lane][iter].t_ser)>0:
										...
										temp_ind = functions.find_index(sim_obj.spawned_veh[lane][iter], time_track)
										assert temp_ind!=None,f'time_track:  {sim_obj.spawned_veh[lane][iter].t_ser}'
										if 0>sim_obj.spawned_veh[lane][iter].p_traj[temp_ind]>=data_file.int_start[lane]:pass
										else: 
											print("ERROR in red signal")
											assert False
									else: pass
									...
									pre_v = None
									if n >1 and iter >0 : pre_v = copy.deepcopy(sim_obj.spawned_veh[lane][iter-1])
									sim_obj.spawned_veh[lane][iter], success = safe_set_map_bisect.acc_map(v, pre_v, time_track, flag = 'red')
									if success == False: ##OVERRIDE
										override_veh.append(v.id)
										override_lane[lane] = copy.deepcopy(override_veh)

										if v.id in override_veh:		  
											f = open("debug_over.txt", "a")
											f.write(f'\n entry ID:{v.id}, over:{override_lane},  at time:{time_track}')
											f.close()


										if pre_v == None or pre_v.id in override_lane[lane]:


											####### MAX PHYS ACCELERATION USED ##########
											####### SAFE SET NOT USED  ##########

											#############################################################################
											# sim_obj.spawned_veh[lane][iter], success = safe_set_map_bisect.green_map(v, pre_v,time_track, over='override')
											# override_veh.append(v.id)

											sim_obj.spawned_veh[lane][iter] = copy.deepcopy(functions.max_physical_acc(sim_obj.spawned_veh[lane][iter], time_track))
											sim_obj.spawned_veh[lane][iter].t_ser.append(round((time_track + (data_file.dt)), 1))
											sim_obj.spawned_veh[lane][iter].sig[time_track]='orange_inter'
										else: assert False
							...
							if len(override_veh)>0: override_lane[lane] = copy.deepcopy(override_veh)
				...
				for _ in data_file.lanes:
					for veh in sim_obj.spawned_veh[_]:
						if  (data_file.L + veh.intsize) >= veh.p_traj[-1] > data_file.int_start[_]:
							...
							# if (veh.sig[time_track] == 'red') and dict_sig[_] != 'R':
							# 	print(f'robot wrong traj-1, id: {veh.id}, sig: {veh.sig[time_track]}, dict_sig: {dict_sig[_]}, pos: {veh.p_traj} '  )
							# 	exit()
							# elif dict_sig[_] == 'R' and veh.sig[time_track] not in {'red', 'orange', 'orange_inter'}:    #and _ not in override_lane :
							# 	print(f'\n robot wrong traj-2, lane:{_} id: {veh.id}, time:{time_track}, sig: {veh.sig[time_track]}, dict_sig: {dict_sig[_]}, \n pos: {veh.p_traj}, overeide:{override_lane} \n sig_stat:{veh.sig}, pos:{veh.pos_status}'  )
							# 	exit()
							# else: pass

							if (veh.sig[time_track] == 'red' and dict_sig[_] == 'G'):
				...
				# assert all ([ veh.sig[time_track] == 'red' and dict_sig[lane] == 'R' for lane in data_file.lanes for veh in sim_obj.spawned_veh[lane]]), \
				# 	f'robot wrong traj, id: {veh.id}, sig: {veh.sig[time_track]}, dict_sig: {dict_sig[lane]}, pos: {veh.p_traj} '    

				# assert all ([ve.sig[time_track]!=None for lane in data_file.lanes for ve in sim_obj.spawned_veh[lane]]),f'time:{time_track}'
				# simliar for done_set also # assert all ([ve.sig[time_track]!=None for lane in data_file.lanes for ve in sim_obj.spawned_veh])
