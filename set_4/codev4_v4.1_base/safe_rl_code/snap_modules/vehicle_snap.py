# This file defines the Vehicle object
...
class Vehicle():
      ...
      def __init__(self, lane_number, init_pos,init_vel, v_min, v_max, u_min, u_max, length, arr_rate_array,prior):
            ...
            self.sig = {key: None for key in range(1, data_file.max_sim_time+1)}   
            ...
            self.p0 =  init_pos   #round(init_pos,1)
            self.v0 = init_vel   #np.random.uniform(self.v_min, self.v_max)
            self.p_traj = []
            self.v_traj = []
            self.u_traj = []
            self.t_ser = []
